-- Demo data as a function, so it is defined in one place and reused by both the
-- seed (supabase db reset / initial load) and the scheduled reset that keeps the
-- public demo tidy. Defining the function inserts nothing on its own; it only
-- runs when called.
